...
export const FAQ = ({ onBook }: { onBook?: () => void }) => {
      ...
      <div className="container mx-auto px-6 max-w-3xl">
        ...
        <div className="space-y-4">
          {FAQS.map((faq, index) => (
            <div
              key={index}
              className={`border rounded-2xl transition-all duration-300 ${openIndex === index ? 'border-brand-gold bg-white' : 'border-brand-dark/5 bg-white/50'}`}
            >
              <button
        ...
        <div className="mt-20 p-10 bg-brand-dark text-white rounded-[3rem] text-center">
          ...
          <button
            onClick={onBook}
            className="bg-brand-gold text-white px-12 py-4 rounded-full hover:bg-white hover:text-brand-dark transition-all duration-300 text-xs tracking-[0.2em] uppercase font-bold"
          >
            Book Appointment
          </button>
        </div>
